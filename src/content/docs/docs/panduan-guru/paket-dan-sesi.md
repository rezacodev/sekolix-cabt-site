...
2. Pilih **Mode Navigasi Seksi**:
   - `Urut`: peserta harus menyelesaikan seksi aktif sebelum pindah ke seksi berikutnya — setelah berpindah tidak bisa kembali
   - `Bebas`: peserta bisa berpindah antar seksi kapan saja selama waktu masih ada
3. Tambahkan seksi — beri nama dan durasi per seksi
4. Assign soal ke masing-masing seksi

:::note
Timer setiap seksi berjalan independen. Pada mode Bebas, timer semua seksi berjalan bersamaan sejak ujian dimulai.
:::
...
| **KKM** | Nilai minimum ketuntasan individual — peserta dianggap tuntas jika nilai akhir ≥ KKM |
...
:::tip
KKM, KKM Klasikal, dan Batas Pengayaan digunakan untuk menghasilkan laporan **Analisis Ulangan** — menentukan peserta mana yang masuk program remidi atau pengayaan. Nilai default KKM adalah 70, KKM Klasikal 65%.
:::
...
:::note
Status Diskualifikasi bersifat permanen. Peserta tidak bisa mengerjakan kembali di sesi yang sama. Hubungi Admin jika diperlukan pengaturan ulang (hapus attempt).
:::
